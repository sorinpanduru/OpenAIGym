...
parser.add_argument("--atari_game", dest="atari_game", type=str, default="SpaceInvaders-ram-v0")
args = parser.parse_args()


env = gym.make(args.atari_game)  # environment info
xs, rs, ys = [], [], []  # State, Reward, Action History
average_reward = None  # Reward for measuring average performance
G = 0
n_episode = 0

print("GameName: ", args.atari_game, "Obs Space: ", env.observation_space.shape, "Action space: ", env.action_space.n)

n_input = env.observation_space.shape[0]            # size of input
n_hidden = 150                                      # number of hidden layer neurons
n_actions = env.action_space.n - 1                  # ?? -1 number of available actions i.e. fire (do nothing)
learning_rate = 1e-3
gamma = .99                                         # discount factor for reward
decay1, decay2 = 0.9, 0.999                         # decay rates for Adam optimizer

game_name = str(args.atari_game.split("-")[0]).lower()
